                ...
                <div v-if="currentPath === '/profile'" class="px-2 py-6 sm:px-6 lg:col-span-6 lg:px-0 lg:py-0">
                    ...
                    <div class="mt-8 p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                        ...
                        <div class="mt-4 grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-4"
                            v-if="props.addresses && props.addresses.length > 0">
                            <div v-for="(address, index) in props.addresses">
                                <div class="relative flex rounded-lg border bg-white p-4 shadow-sm focus:outline-none">
                                    <span class="flex flex-1">
                                        <XMarkIcon @click.prevent="removeAddress(address)"
                                            class="h-5 w-5 text-indigo-500 absolute top-4 right-4 cursor-pointer"
                                            aria-hidden="true" />
                                        <span class="flex flex-col">
                                            <span class="block text-sm font-medium text-gray-900">{{ address.address_line1
                                            }}, {{ address.address_line2 }}</span>
                                            <span class="mt-1 flex items-center text-sm text-gray-500">
                                            </span>
                                            <span class="mt-1 flex items-center text-sm text-gray-500">{{ address.city }},
                                                {{ address.postal_code }}
                                            </span>
                                            <span class="mt-1 flex items-center text-sm text-gray-500">{{ address.country
                                            }}</span>
                                        </span>
                                    </span>
                                </div>
