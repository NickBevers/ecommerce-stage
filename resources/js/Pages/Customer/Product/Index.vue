      ...
      <div class="lg:grid lg:auto-rows-min lg:grid-cols-12 lg:gap-x-8">
        ...
        <div class="mt-8 lg:col-span-7 lg:col-start-1 lg:row-span-3 lg:row-start-1 lg:mt-0">
          ...
          <div class="flex flex-row gap-4">
           <div class="flex flex-col w-1/3 gap-4 h-[38rem] overflow-y-scroll no-scrollbar relative">
          ...
          <div class="flex flex-col w-2/2 h-fit w-full h-[38rem] rounded-md overflow-hidden ">
                <img :src="imagePreview" class="h-full w-full object-cover object-center">
          </div>
        </div>
        </div>
        ...
        <div class="mt-8 lg:col-span-5">
          <form>
              ...
              <div v-if="!showAll" class="overflow-hidden">
                ...
                <Link :href="`/product/${variation.sku}`">
                  <img class="h-32 w-20  inline hover:scale-125 duration-500 transition-all rounded-md object-cover"
                    :src="variation.product_images[0].image_link"
                    alt="variation.name" />
                </Link>
              </div>
              </div>
            ...
            <div v-if="props.variations.length > 4 && !showAll" class="flex text-center justify-center content-center mt-1 object-center rounded-md">
              <button class="h-32 w-20  border-2 border-indigo-300 rounded-md hover:border-indigo-600 text-indigo-300 hover:text-indigo-600 duration-500 transition-all" @click.prevent="showAll = !showAll">+{{props.variations.length - 4}}</button>
            </div>
          ...
          <div class="mt-10" v-if="props.sku.product.description">
            ...
            <div class="prose prose-sm mt-4 text-gray-500">
              <p>{{ props.sku.product.description}}</p>
            </div>
          </div>

          <div class="mt-8 border-t border-gray-200 pt-8" v-if="props.sku.product.extra_info">
            <h2 class="text-sm font-medium text-gray-900">Extra Info</h2>

            <div class="prose prose-sm mt-4 text-gray-500">
              <p>{{ props.sku.product.extra_info }}</p>
            </div>
          </div>
